...
// ── STATE
var rows=[], curLang='all', curQ='', isAdmin=false;
var curSnip=null, editId=null, delId=null, toastTm=null;
// BUG FIX: wrap localStorage access in try/catch — can throw in private mode / storage full
var liked=new Set();
...
function apiFetch(url,method,data,timeoutMs,creds){
  ...
  // BUG FIX: credentials default should be 'same-origin' for regular endpoints, 'include' only for admin
...
// ── BOOT
(async function(){
  // Admin verify — non-blocking, short timeout
  try{
...
async function load(retries){
  ...
  for(var i=0;i<=retries;i++){
    try{
      ...
      var d=await r.json();
      // BUG FIX: validate response is array before assigning
...
function makeRowHTML(s){
  ...
  // BUG FIX: escape tag content, cap at 2 tags
...
async function openDetail(id){
  // BUG FIX: id type coercion — DB might return string or number
  var nid=Number(id);
  var s=rows.find(function(r){return Number(r.id)===nid;});
  if(!s) return;
  curSnip=s;

  // View increment (fire-and-forget, don't block UI)
...
async function toggleLike(id){
  ...
  var s=rows.find(function(r){return Number(r.id)===nid;});
  if(!s) return;
  var k=String(nid), wasLiked=liked.has(k);
  // BUG FIX: optimistic UI update for instant feedback
...
function copyCode(){
  ...
  navigator.clipboard.writeText(curSnip.code)
    ...
    .catch(function(){
      ...
      ta.value=curSnip.code;ta.style.cssText='position:fixed;opacity:0;top:0;left:0';
...
function downloadCode(){
  ...
  // Blob download works on all mobile browsers regardless of CORS
  try{
    var ext=LANG_EXT[curSnip.language]||'txt';
    var fname=(curSnip.title||'snippet').toLowerCase()
      .replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,50)||'snippet';
...
async function submitNew(){
  ...
  var f={
    author:gv('n-author'),title:gv('n-title'),
    description:gv('n-desc'),tags:gv('n-tags'),
    language:gv('n-lang')||'JavaScript',
  ...
  try{
    ...
    closeOv('ov-new');
    // BUG FIX: clear all form fields after success
...
function openEditM(id){
  ...
  editId=nid;
  sv('e-title',s.title||'');
  sv('e-lang',s.language||'JavaScript');
  sv('e-desc',s.description||'');
  sv('e-tags',tagArr(s.tags).join(', '));
  // BUG FIX: if code not loaded yet, show empty (will be saved as-is which is wrong)
  // Only populate code field if code exists
  sv('e-code',s.code||'');
  sv('e-key','');
  var ee=$(  'er-ekey');if(ee) ee.textContent='';
  var ew=$(  'editKeyWrap');if(ew) ew.style.display=isAdmin?'none':'block';
  // If code not loaded, fetch it first
  if(!s.code){
    var ce=$(  'e-code');
...
async function submitEdit(){
  ...
  try{
    ...
    // BUG FIX: validate required fields before sending
    if(!body.title.trim()){toast('Judul tidak boleh kosong','fail');return;}
...
function doSearch(q){
  ...
  // BUG FIX: sync both search inputs without causing infinite loop
...
function closeOv(id){
  ...
  if(id==='ov-detail') history.replaceState(null,'','/app');
  if(id==='ov-admin'){
...
document.addEventListener('keydown',function(e){
  if(e.key==='Escape'){
    // BUG FIX: close only the topmost open modal, not all at once
